import { Block, colors } from "./Block";
...
export class Board {
  ...
  public addBlock = (block: Block, x: number, y: number): void => {
    block.rotationShape[block.rotation].forEach((row, rowI) => {
  ...
  public canBlockMoveRight = (block: Block, x: number, y: number): boolean => {
    return block.rotationShape[block.rotation].every((row, rowI) => {
  ...
  public canBlockRotate = (block: Block, x: number, y: number): boolean => {
    console.log(block.getNextRotate());
    return block.rotationShape[block.getNextRotate()].every((row, rowI) => {
      return row.every((element, elementI) => {
        if (y + rowI >= ROWS) {
          return false;
        }
